...
if [ "$CURRENT" = "0.900000" ]; then
  ...
  sed -i -e '37s/false/true/' -e '34s/40/3/' -e '35s/20/3/' -e '38s/{ 0, 0 }/{ 12, 12 }/' -e '10s/0/2/' -e 's/gaps_in = 10/gaps_in = 20/' -e 's/gaps_out = 20/gaps_out = 30/' -e '29s/active_opacity = 0.9/active_opacity = 1.0/' -e '30s/inactive_opacity = 0.7/inactive_opacity = 1.0/' -e 's/rounding = 5/rounding = 0/' -e 's/rounding_power = 10/rounding_power = 0/' /home/hamato/.config/hypr/modules/decorations.lua
  ...
  sed -i -e '37s/true/false/' -e '34s/3/40/' -e '35s/3/20/' -e '38s/{ 12, 12 }/{ 0, 0 }/' -e '10s/2/0/' -e 's/gaps_in = 20/gaps_in = 10/' -e 's/gaps_out = 30/gaps_out = 20/' -e '29s/active_opacity = 1.0/active_opacity = 0.9/' -e '30s/inactive_opacity = 1.0/inactive_opacity = 0.7/' -e 's/rounding = 0/rounding = 5/' -e 's/rounding_power = 0/rounding_power = 10/' /home/hamato/.config/hypr/modules/decorations.lua
